...
【解题思路】
首先理解题目，这是个树的搜索，题目要求搜索出总值等于给定输入的所有路径，并且将找到的路径按照“非递增”的方式输出。这就涉及到两个考点：1搜索树，
1 不同的队列按照非递增排序。结合题目附带的代码，main函数可以设计出三段内容：
1 使用合适的数据结构保存输入的数据；
2 利用合适的算法找出所有路径；
3 找出所有总值等于给定输入s的路径集合（这一步可以在步骤2一并实现，不过会增加步骤2的复杂度，容易增加调试时间），
   并按“非递增”方式输出这些路径的值。题目给定了一些条件，根节点固定是“00”，用例保证至少有一条路径满足要求，可以简化解题步骤。
1、	使用合适的数据结构保存输入的数据
这一步决定了步骤2的算法的复杂度，由于题目中的数不是常规的二叉树，又是单向查找，终结条件就是找到的节点是否叶子节点，可以采用广度优先的算法。
将根节点和下一级叶子节点的集合的映射保存起来，采用广度优先算法遍历搜索。
至于每个编号的值，题目明确了是按照编号顺序赋值，可以直接采用一个数组把输入的值依次获取到。
2、	利用合适的算法找出所有路径
步骤1决定了使用广度优先算法，剩下的就是实现方式的事了，可以采用遍历或者递归，递归一般条件设置不好调试很麻烦，因此采用偏暴力的遍历方法。
首先初始化一个根节点作为起始搜索结果paths，然后基于这个起始搜索结果，根据节点的子节点集合进行循环遍历
3、	找出所有总值等于给定输入s的路径集合，并按“非递增”方式输出这些路径的值
Paths找到之后，判断是否相等就很简单了，难点在要对待输出paths的集合进行排序，参照输入样例（下图），按照数组的列的顺序依次比较，大的排前面。
这个算法如果用暴力破解，没想出来，不过下面这个图，横着看很像4个值很大的数字，那么我们的算法就是把这4个path数组，变成4个double的值，然后排序即可。

/*
第一行分别为节点个数N（N < 100） 非叶子界定啊个数 M（< N）总容量s (<2^30)
第二行按照节点编号顺序给出 N个节点的管理容量
随后的M行已 ID K ID【1】 ID【2】。。。。。ID【k]的形式给出
ID是一个非叶节点的2位数编号 K是起子节点的个数，后面K个数字是一系列子节点的编号
