...
2. 要记住二叉搜索树的性质，正常的中序遍历就是升序，如果中序遍历时，先遍历右子树，那么就变成降序了。
#### 代码
```
int calc(struct TreeNode* cur, int sum)
{
    if (cur == NULL) {
        return sum; // 这里的sum 和下面的rightsum实际上是一样的大小。对于在回溯的过程中。
    }
    int rightSum = calc(cur->right, sum);
    cur->val += rightSum; // 这里是把右子树的和进行累加. 同时也改变了当前节点的值。
...
3. 第三步： 计算右子树，即处理前序遍历的右半部分 和 中序遍历的右半部分。preorder + 1 + pos， 即 从 3开始， 长度就是总长度减去 (1 + pos);
...
#### 思路
1. 常规思路就是把这个二叉搜索数中序遍历出来，然后逐个匹配给定的p, 如果匹配到就把它后面的值返回。
2. 采用迭代法，如果当前节点的值比P大，那么当前节点有可能是P的后继，先把结果记录下来，下来去当前节点的左子树上找；如果当前节点的值比P小，那么直接到右子树上去找。
#### 代码
```
方案I:
int gCnt;
void dfs(struct TreeNode* root, struct TreeNode** queue)
{
    if (root == NULL) {
        return;
    }
...
struct TreeNode* inorderSuccessor(struct TreeNode* root, struct TreeNode* p) {
    struct TreeNode** queue[CNT] = {NULL};
    dfs(root, queue);
    for (int i = 0; i < gCnt; i++) {
        if (p == queue[i] && i < gCnt - 1) {
            return queue[i + 1];
        }
    }        
    return NULL;
...
#### 思路
1. 理解题意，有2个方法，next每次都获取当前二叉搜索中最小的元素，取过一次后，刚才那个元素就已经不存在； hasnext,是用来判断当前是否已经完全遍历完了整个二叉搜索树。
2. 常规方法是，先使用中序遍历，把数据放在数组中，另外设置2个变量用来保存总的数据个数和当前访问的个数。然后题意中需要的这2个方法就很好解决
3. 进阶方案：2中需要O(n)的存储空间，我们可以用O(h) h表示高度的空间复杂度来解决。 我们需要一个栈，我们把左子树依次入栈，next 方法就对应pop出去的值。pop出来的节点如果有右子树的话，我们把右子树插入到左子树中。 hasnext 方法使用 栈是否为空来判断。
#### 代码
```
方案I:
#define CNT 100001
...
void dfs(struct TreeNode* root, int* data, int* cnt)
{
    if (root == NULL) {
        return;
    }
    dfs(root->left, data, cnt);
    data[*cnt] = root->val;
    (*cnt)++;
    dfs(root->right, data, cnt);
}

BSTIterator* bSTIteratorCreate(struct TreeNode* root) {
    BSTIterator* obj = (BSTIterator*)malloc(sizeof(BSTIterator));
    memset(obj->data, 0, sizeof(int) * CNT);
    obj->curCnt = 0;
    obj->totalCnt = 0;
    dfs(root, &obj->data, &obj->totalCnt);
    return obj;
}

int bSTIteratorNext(BSTIterator* obj) {
    int data = obj->data[obj->curCnt];
    obj->curCnt++;
    return data;
}

bool bSTIteratorHasNext(BSTIterator* obj) {
    return obj->curCnt < obj->totalCnt;
}
...
void push(Stack* obj, struct TreeNode* node)
{
    obj->top++;
    obj->data[obj->top] = node;
}

struct TreeNode* pop(Stack* obj)
{
    struct TreeNode* node = obj->data[obj->top];
    obj->top--;
    return node;
}
...
void insertLeft(BSTIterator* obj, struct TreeNode* cur)
{
    while (cur) {
        push(obj->nodeStack, cur);
        cur = cur->left;
    }
}
...
BSTIterator* bSTIteratorCreate(struct TreeNode* root) {
    ...
    struct TreeNode* cur = root;
    insertLeft(obj, cur); // 把左子树完全入栈
    return obj;
}

int bSTIteratorNext(BSTIterator* obj) {
    struct TreeNode* data = pop(obj->nodeStack);
    struct TreeNode* dummy = data->right; // pop 出来的节点如果有右子树，则把右子树入栈，再把右子树的所有左子树都入栈
    if (dummy) {
        insertLeft(obj, dummy);
    }
    return data->val;
}

bool bSTIteratorHasNext(BSTIterator* obj) {
    return !isEmpty(obj->nodeStack);
}
...
### leetcode 94
### 思路
1. 对于中序遍历，通过使用标记法，来把二叉树的迭代访问方法写成统一形式。
### 代码
```
#define MAX_CNT 1000
typedef struct {
    struct TreeNode* buff[MAX_CNT];
    int top;
} Stack;

Stack* Init(void)
{
    Stack* obj = (Stack*)calloc(1, sizeof(Stack));
    obj->top = -1;
    return obj;
}
...
{
    obj->top++;
    obj->buff[obj->top] = element;
}

struct TreeNode* GetTop(Stack* obj)
{
    return obj->buff[obj->top];
}

struct TreeNode* Pop(Stack* obj)
{
    struct TreeNode* element = NULL;
    element = obj->buff[obj->top];
    obj->top--;
    return element;
}

int* inorderTraversal(struct TreeNode* root, int* returnSize){
    int* res = (int*)calloc(MAX_CNT, sizeof(int));
    *returnSize = 0;
    ...
    Push(obj, root);
    while (!Empty(obj)) {
        struct TreeNode* cur = GetTop(obj);
        if (cur != NULL) {
            Pop(obj); // 先弹出，避免重复操作
            if (cur->right) { // 右
                Push(obj, cur->right);
            ...
            Push(obj, cur); // 中
            Push(obj, NULL); // 标记法，cur已经访问过了，使用null标记一下。
            if (cur->left) { // 左。
                Push(obj, cur->left);
            }
        } else {
            Pop(obj);
            struct TreeNode* dat = GetTop(obj);
            Pop(obj);
            res[*returnSize] = dat->val;
            (*returnSize)++;
        }
    }
    return res;
