...
### 二叉树的三种方式遍历
思路：
使用递归很方便的遍历二叉树，这里需要注意的是，三种方式遍历方式，在递归上都是左子树 和 右子树在执行，获取相关元素都是在根节点上执行。下面以中序遍历说明
### 代码
```
int* result = NULL;
...
二叉树在构建时，一定要在创建一个节点时，要把他的左子树和右子树先设置成NULL，然后在后续的过程中再通过创建节点连接到其父节点的左子树或者右子树上。
...
#### 后序遍历
   ...
   2.先把根节点放入栈A，然后弹出根节点放在B栈
   3.如果弹出的节点有左子树，则把左子树入栈A
   4.如果弹出的节点有右子树，则把右子树入栈A
   5.如果栈不为空时，A出栈，入栈B
   6.栈B逐个出栈打印。
#### 中序遍历
   1.使用一个栈。
   2.把树的左子树一次入栈，直到没有左子树就停下来
   3.弹出直接打印，并走到这个弹出节点的右子树上。
   4.执行步骤2.
