...
2. 单层循环的做法：
#### 对于排列问题（对于不同顺序的结果都认为是不同结果），在for循环的过程中，变量每次都是从0开始的
#### 对于组合问题(对于顺序没有要求)for循环遍历的时候是从上一次结束的位置开始，所以要从idx开始循环。
#### 对于单层循环要操作的步骤较少时，可以直接操作，然后回溯，不用for循环。
#### 对于最终结果要求包含重复数字的时候，进行下一次回溯的时候，不用增加遍历索引。
#### 注意在回溯时，在backtrace()调用前对变量进行加加 然后 在backtrace()后执行减减操作，等同与直接在backtrace(cnt + 1,,,,) 这种操作
