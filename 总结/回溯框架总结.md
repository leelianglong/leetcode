...
## 总结
1. 终止条件要确认完整
2. 单层循环的做法：a、对于排列问题（对于不同顺序的结果都认为是不同结果），在for循环的过程中，变量每次都是从0开始的
b、对于组合问题(对于顺序没有要求)for循环遍历的时候是从上一次结束的位置开始，所以要从idx开始循环。c、对于单层循环要操作的步骤较少时，可以直接操作，然后回溯，不用for循环。d、对于最终结果要求包含重复数字的时候，进行下一次回溯的时候，不用增加遍历索引。
