...
* 什么时候是对操作的所有数据进行遍历，什么时候对数据操作的可能性进行遍历？例如目标和题目要对操作可能性进行遍历，而不是对所有数据
---
### 题目 生命的游戏
### 思路：
...
### leetcode 582
### 思路
1. 当前题意是给定2个数组，child[i], parent[i], 他们分别存储的是进程ID，parent[i] 是child[i]的父进程。
2. 题目要求指定一个父进程号删除，请返回包含该父进程的所有子进程的序列。
3. 起初想建树，发现没有办法，参考题解，借助hash思路，把父节点和它的所有孩子节点，建立关系，存储在一个hash节点思路。
4. 删除时，先把父节点放到返回结果里面，然后使用该父节点作为key，在hash链表中去查找，如果找不到就直接退出，如果找到了，那么就递归的调用当前函数，以当前父节点的所有孩子节点作为参数。
### 代码
```
#define CNT 50000
struct HashObj {
